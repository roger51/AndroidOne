...
public class TryRefreshableView extends LinearLayout {
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    Log.d("TryPullToRefreshScrollView", "view.getMeasureHeight:"+ getMeasuredHeight());
    ...
    public void FooterPrepareForRefresh() {
        resetFooterPadding();
        mfooterViewText.setText(R.string.pull_to_refresh_footer_refreshing_label);
        ...
        if (getChildCount() > 1) {
            ...
            if (childView instanceof ListView) {
                ...
                if (((ScrollView) childView).getScrollY() == 0 && diff > 0) {
                    ...
                    Log.i("other", "外框处理");
                    return true;
                } else if ((((ScrollView) childView).getChildAt(0) .getMeasuredHeight() <= ((ScrollView) childView).getScrollY() + getHeight() && diff < 0)) {// 底部上拉
                    Log.i("other", "外框处理2");
                    nowpull = 1;
